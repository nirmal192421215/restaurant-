  ...
  return (
    <>
      ...
      <motion.a
        ...
        className="fixed bottom-28 right-6 z-40 bg-[#25D366] text-white p-4 rounded-full shadow-[0_8px_30px_rgb(37,211,102,0.4)] transition-transform flex items-center justify-center group"
      >
          ...
          {navItems.map((item) => (
            ...
            >
              <motion.div 
                ...
                className="w-10 h-10 rounded-xl flex items-center justify-center transition-colors group-active:bg-[var(--color-brand-orange)]/10"
              >
              ...
              <AnimatePresence>
                {item.badge !== undefined && item.badge > 0 && (
                  <motion.span 
                    ...
                    animate={{ scale: 1 }}
                    exit={{ scale: 0 }}
                    className="absolute top-1 right-1/4 w-5 h-5 bg-[var(--color-brand-orange)] text-white text-[10px] font-bold rounded-full flex items-center justify-center shadow-lg border-2 border-white"
                  >
                    {item.badge}
                  </motion.span>
                )}
